...
function NavBarForCarryForwardedPage(props) {
  ...
  var [filteredFoodBasedOnSearchInput, setFilteredFoodBasedOnSearchInput] =
    useState([]);
  ...
  function filterTheFoodItem(searchKeyword) {
    let filteredNonVegItemsBasedOnKeyword = props.all_data.reduce(
      (accumulator, objectInCurrentLoop) => {
        let nonvegsSearch = objectInCurrentLoop.menu_available.non_veg.filter(
          (f) => {
  ...
  return (
    <div className='navBarMainContainer'>
      ...
      <div className="navbarprofileholder">
      <UserProfileUI />
      </div>
